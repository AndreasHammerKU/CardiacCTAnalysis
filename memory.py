...
class ReplayMemory(object):
    def __init__(self, max_size, state_shape, number_of_samples, history_len, agents):
        self.number_of_samples = number_of_samples
        self.max_size = int(max_size)
        self.state_shape = (number_of_samples, *state_shape)
        self.history_len = int(history_len)
        self.agents = agents
        try:
            self.state = np.zeros(
                (self.agents, self.max_size) + self.state_shape, dtype='uint8')
    ...
    def sample(self, batch_size):
        ...
        return (np.array(states)[:,:,-1,:,:,:,:], np.array(actions)[:, :, -1],
                np.array(rewards)[:, :, -1], np.array(next_states)[:,:,-1,:,:,:,:],
                np.array(isOver)[:, :, -1])
