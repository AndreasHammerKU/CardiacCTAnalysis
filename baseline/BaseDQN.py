...
class Network3D(nn.Module):
    def __init__(self, agents, frame_history, number_actions, location_dim=3, xavier=True):
    ...
    def forward(self, state, location):
        """
        Input:
        - state: (batch_size, agents, frame_history, *image_size)
        - location: (batch_size, agents, location_dim)
        
        Output:
        - (batch_size, agents, number_actions)
        """
        batched = len(state.shape) == 5  # Check if batch dimension exists
        if batched:
