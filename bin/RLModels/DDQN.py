...
class DDQN(RLModel):
    ...
    def optimize_model(self, memory, batch_size=32):
        ...
        with torch.no_grad():
            ...
            # Use policy_net to select best actions for next states
            next_action_indices = self.policy_net(next_states, next_locations) \
                .view(-1, self.agents, self.n_actions) \
                .argmax(-1, keepdim=True)

            # Use target_net to evaluate those actions
            next_q_values = self.target_net(next_states, next_locations) \
                .view(-1, self.agents, self.n_actions) \
                .gather(2, next_action_indices) \
                .squeeze(-1)
        
        next_q_values = (1 - dones.squeeze(-1)) * next_q_values

        # Bellman equation with averaged rewards
        expected_state_action_values = (next_q_values * self.gamma) + rewards
